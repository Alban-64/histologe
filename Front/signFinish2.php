...
$x=0; $p=0; $z=0;
 ...
 $etage=0; $apt=''; $isCaf=0;
if(isset($_SESSION['CAF'])) $isCaf=$_SESSION['CAF'];
...
// echo $mail.'-'.$tel.'-'.$type.'-'.$numAd.'-'.$libAd.'-'.$cpAd.'-'.$ville.'-'.$etage.'-'.$apt.'-'.$infoProprio.'-'.$description.'-'.$cgu.'-'.$adults.'-'.$enfants.'-'.$nom.'-'.$prenom.'-'.$surface;
 ...
 $sign = $infosForm->setSignalement($mail,  $tel, $type, $numAd, $libAd, $cpAd, $ville, $etage, $apt, $infoProprio, $description, $cgu, $adults, $enfants, $nom, $prenom, $surface, $social, $isCaf, $depart);
 ...
 foreach($_FILES as $key=>$value){
     //   echo 'file : '.$key.' value : '.$value;
            ...
            if($_FILES['file'.$y]['tmp_name']!='') {
                ...
                {
                    $_SESSION['error'] = "Le fichier est introuvable";
                ...
                {
                    $_SESSION['error'] = "Impossible de copier le fichier";
                    header("Location:Error");
                    exit;
                }
            }
