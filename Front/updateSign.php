...
if($action=='proprio') {
    ...
    $infosForm->addSuivi($idSign,'Le demandeur certifie avoir averti son propriétaire.', 'Demandeur', $_SERVER['REMOTE_ADDR'], 'off',0 );
}
if($action=='proprioKO') {
    $infosForm->updateSignalement('proprio_info', $idSign, 'n');
    $infosForm->addSuivi($idSign,'Le demandeur annule avoir certifié averti son propriétaire.', 'Demandeur', $_SERVER['REMOTE_ADDR'], 'off',0 );
}

if($action=='nrj') {
    $infosForm->updateSignalement('infoNrj',$idSign,$modeNrj);
    $infosForm->addSuivi($idSign,'Le demandeur a renseigné son mode énergie.', 'Demandeur', $_SERVER['REMOTE_ADDR'], 'off',0 );
}
...
if($action=='rdv') {
    ...
    $infosForm->addSuivi($idSign,'Le demandeur a proposé un créneau pour une visite à distance.', 'Demandeur', $_SERVER['REMOTE_ADDR'], 'off',0 );
    $sign=$infosForm->getSignById($idSign);
    $mail=$infosForm->getUserBoById($sign[0]->idUserBoVisit);
...
if($action=='propVisit') {
    ...
    $infosForm->addSuivi($idSign,'Proposition d\'une visite à distance.', $_SESSION['user'], $_SERVER['REMOTE_ADDR'], 'off',$_SESSION['idPartenaire'] );
...
if($action=='invitTeams') {
    ...
    $infosForm->addSuivi($idSign,'Invitation Teams transmise pour visite à distance', $_SESSION['user'], $_SERVER['REMOTE_ADDR'], 'off',$_SESSION['idPartenaire'] );
}
...
if($action=='visitOk') {
    ...
    $infosForm->addSuivi($idSign,'Visite à distance réalisée.', $_SESSION['user'], $_SERVER['REMOTE_ADDR'], 'off',$_SESSION['idPartenaire'] );
}
